...
int main(void){
	...
	uint8_t p0_control_buf[MAX_P0_LEN];				//¶¨Òå¿ØÖÆÊý¾Ý»º³åÇø
	
	SystemInit();															//ÏµÍ³Ê±ÖÓ³õÊ¼»¯
	HW_Init();																//°å¿¨Ó²¼þ³õÊ¼»¯
	Printf_SystemRccClocks();									//ÏÔÊ¾ÏµÍ³Ê±ÖÓ
	SW_Init();															  //Çå¿Õ½ÓÊÜÊý¾Ý°ü
	while(1){
		KEY_Handle();														//°´¼üÉ¨Ãè
		GizWits_MessageHandle(p0_control_buf, sizeof(WirteTypeDef_t));
		if(p0Flag == 1){
			memcpy((uint8_t *)&WirteTypeDef, p0_control_buf, sizeof(WirteTypeDef_t));		//¿½±´p0_control_bufÊý¾Ýµ½WirteTypeDef
...
void HW_Init(void){
	...
	UARTx_Init();								//´®¿Ú³õÊ¼»¯
  RGB_KEY_GPIO_Init();				//RGBÊ±ÖÓÏß³õÊ¼»¯
	RGB_LED_Init();							//RGB³õÊ¼»¯
	LED_GPIO_Init();						//LED³õÊ¼»¯
	KEY_GPIO_Init();						//°´¼ü³õÊ¼»¯
