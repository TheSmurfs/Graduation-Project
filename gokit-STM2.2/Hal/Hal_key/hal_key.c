...
void KEY_GPIO_Init(void){
    ...
    RCC_APB2PeriphClockCmd(GPIO_KEY1_CLK | GPIO_KEY2_CLK | GPIO_KEY3_CLK, ENABLE);		//使能按键时钟

    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;																	//设置IO口速度
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;																			//配置上拉输入模式

    GPIO_InitStructure.GPIO_Pin = GPIO_KEY1_PIN;																			//选择按键1
    GPIO_Init(GPIO_KEY1_PORT, &GPIO_InitStructure);																		//初始化按键1

    GPIO_InitStructure.GPIO_Pin = GPIO_KEY2_PIN;																			//选择按键2	
    GPIO_Init(GPIO_KEY2_PORT, &GPIO_InitStructure);																		//初始化按键2

    GPIO_InitStructure.GPIO_Pin = GPIO_KEY3_PIN;																			//选择按键3
    GPIO_Init(GPIO_KEY3_PORT, &GPIO_InitStructure);																		//初始化按键3
}
...
uint8_t Get_Key(void){
    uint8_t ReadKey;
    if(!GPIO_ReadInputDataBit(GPIO_KEY1_PORT,GPIO_KEY1_PIN)){				//判断按键1是否按下
        ReadKey |= PRESS_KEY1;																			//设置键值
    }
    if(!GPIO_ReadInputDataBit(GPIO_KEY2_PORT,GPIO_KEY2_PIN)){				//判断按键2是否按下
        ReadKey |= PRESS_KEY2;																			//设置键值
    }
    if(!GPIO_ReadInputDataBit(GPIO_KEY3_PORT,GPIO_KEY3_PIN)){				//判断按键3是否按下
        ReadKey |= PRESS_KEY3;																			//设置键值
    }
